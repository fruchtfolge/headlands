...
const headland = {
  lineStrings(poly, options = {}) {
    ...
    poly.geometry.coordinates[0].forEach((currentCoord, coordIndex) => {
      ...
      const distToLastPoint = distance(lastPoint, currentCoord) * 1000;
      ...
      if (curLine.length === 1) {
        refAngle = bearing(lastCoord, currentCoord); // angleCoords(lastCoord,currentCoord)
      ...
      if (coord.aboveminCoordDistance && coord.exceedsTurnAngle) {
        ...
        if (uncertainPoints.length) {
          ...
          refAngle = bearing(uncertainPoints[0], currentCoord);

          uncertainPoints = [];
        } else {
          lineStrings.push(curLine);
          refAngle = bearing(lastPoint, currentCoord);
        ...
        const actAngle = bearing(lastPoint, currentCoord);
    ...
    if (curLine.length && lineStrings.length) {
      ...
      const initAngle = bearing(lineStrings[0][0], lineStrings[0][1]); //angleCoords(lineStrings[0][0],lineStrings[0][1])
  ...
  polygons(poly, options) {
    ...
    try {
      polygons = lineStrings.features.map(ls => {
        ...
        let headland = buffer(ls, options.width / 1000);
...
function checkIntersections(polys) {
  ...
  for (let i = 0; i < polys.length; i++) {
    ...
    try {
      ...
      const intersectionPoly = multiPolygon(intersectionCoords);
      const intersectionArea = area(intersectionPoly);
      const curHeadlandArea = area(curHeadland);
...
function getCoordDetails(currentCoord, lastCoord, refAngle, minCoordDistance, maxAngle) {
  const curDistance = distance(lastCoord, currentCoord) * 1000;
  const curAngle = bearing(lastCoord, currentCoord);
...
function extendLineString(ls, width, lastPoint) {
  ...
  const newPointDirection = bearing(point1, point2);
  const newPoint = destination(point2, width / 1000, newPointDirection);
...
function interpolate(poly, minCoordDistance) {
  ...
  segmentEach(poly, (currentSegment, featureIndex, multiFeatureIndex, geometryIndex, segmentIndex) => {
...
function polygonCut(polygon, line, idPrefix) {
  ...
  if ((booleanWithin(point(lineCoords[0]), polygon) ||
  ...
  for (i = 0; i <= 1; i++) {
    ...
    for (j = 0; j < clipped.geometry.coordinates.length; j++) {
      polyg = polygon$1(clipped.geometry.coordinates[j]);
    ...
    cutPolyGeoms.forEach(function(geometry, index) {
      ...
      cutFeatures.push(polygon$1(geometry, {
        id: id
      }));
    });
  }
